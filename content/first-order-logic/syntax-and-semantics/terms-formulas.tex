...
\olsection{fol}{syn}{frm}{Terms and \printtoken{P}{formula}}
...
\item If $f$ is an $n$-place !!{function} and $t_1$, \dots, $t_n$
  are terms, then $\Term{f}{t_1, \ldots, t_n}$ is a term.
\tagitem{limitClause}{
...
understand $\Term{f}{t_1, \ldots, t_n}$ as just $f$ by itself if $n =
0$.
\end{explain}
...
$\Term{\Obj f^2_0}{t_1, t_2}$, $\Atom{\Obj A^2_0}{t_1, t_2}$ and $\Term{\Obj f^1_0}{t}$,
respectively.
...
\begin{lem}[\emph{Principle of induction on terms}]
    ...
    \begin{enumerate}
        ...
        %
        \item it holds for $\Term{f}{t_1,\dotsc,t_n}$ whenever it holds for
